...
button.btn.btn-primary.gal {
   ...
   <?php foreach ($l_select_letter as $l_seletter_lists) {
	 $sevalue=$l_seletter_lists->letter_id;
	 $letters=$l_seletter_lists->letter_contents;
	
   ?>
   <?php }?> 

   <?php foreach ($l_all_letter as $l_all_letter) {
	 $letter=$l_all_letter->message;
	 $letter_id=$l_all_letter->emailtemplateid;
   ?>
   <?php }?> 
   
   
 <?php foreach ($l_view_data as $view_datas) {
	  $id=$view_datas->id;
?>
<?php }?>  
<div id="wrapper">   
						...
						<div class="project_letter">
						   <div class="row">
								...
								<div class="col-md-12">
									...
									<form action="<?php echo admin_url('leads/letter_update/'); ?><?php if($sevalue!=null){ ?><?php echo  $sevalue; ?><?php }else{ ?><?php echo  $letter_id; ?><?php } ?>/<?php echo $id ?>"  method="post" enctype="multipart/form-data" id="letter11">
									...
									<div class="letter-selecton" style="margin-bottom: 12px;">
									 ...
									 <label for="letteron">Select Letter</label>
									  <select class="selectpicker form-control" id="letteron" name="selectitem">
										   <?php foreach ($l_email_letter as $l_email_letter_lists) { ?>			

                                              <?php   if($sevalue==$l_email_letter_lists->emailtemplateid){ ?>
													<option selected value="<?php echo $l_email_letter_lists->emailtemplateid ?>"><?php echo $l_email_letter_lists->name ?></option> 
												<?php }else{ ?>
													 <option value="<?php echo $l_email_letter_lists->emailtemplateid ?>"><?php echo $l_email_letter_lists->name ?></option>
												 <?php } ?>									   
		...
		tinymce.activeEditor.setProgressState(false, 500)
		  var s='<?php echo $id ?>';
		   var value=$(this).val();
			...
			if(frm) {
			   frm.action =admin_url+'leads/letter_update/'+ value+'/'+ s; 
			}
