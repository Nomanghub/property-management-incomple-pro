...
button.btn.btn-primary.gal {
   ...
   <?php foreach ($select_letter as $seletter_lists) {
	 $sevalue=$seletter_lists->letterselect_id;
	 $letters=$seletter_lists->select_contents;
	
   ?>
   <?php }?> 

   <?php foreach ($all_letter as $letter_lists) {
	 $letter=$letter_lists->message;
	 $letter_id=$letter_lists->emailtemplateid;
   ?>
   <?php }?> 
						...
						<div class="project_letter">
						   <div class="row">
								...
								<div class="col-md-12">
									...
									<form action="<?php echo admin_url('projects/letter_update/'); ?><?php if($sevalue!=null){ ?><?php echo  $sevalue; ?><?php }else{ ?><?php echo  $letter_id; ?><?php } ?>/<?php echo  $project->id; ?>"  method="post" enctype="multipart/form-data" id="letter11">
									...
									<div class="letter-selecton" style="margin-bottom: 12px;">
									 ...
									 <label for="letteron">Select Letter</label>
									  <select class="selectpicker form-control" id="letteron" name="selectitem">
										   <?php foreach ($email_letter as $email_letter_lists) { ?>			

                                              <?php   if($sevalue==$email_letter_lists->emailtemplateid){ ?>
													<option selected value="<?php echo $email_letter_lists->emailtemplateid ?>"><?php echo $email_letter_lists->name ?></option> 
												<?php }else{ ?>
													 <option value="<?php echo $email_letter_lists->emailtemplateid ?>"><?php echo $email_letter_lists->name ?></option>
												 <?php } ?>									   
