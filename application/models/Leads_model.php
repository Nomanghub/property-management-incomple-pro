...
class Leads_model extends App_Model
{
	...
	public function l_get_viewdata($id){
		$this->db->where('id', $id);
        return $this->db->get('tblleads')->result();			
	}
	
	public function l_get_select_letter($id){
		$this->db->where('lead_id', $id);
        return $this->db->get('tblleads_letter')->result();			
	}
	
	public function l_get_email_letter(){
		$this->db->where('language', 'english');
		$this->db->where('active', 1);
		$this->db->where('type', 'letters');
		$this->db->where('slug', 'lead-letter');
		$this->db->order_by('emailtemplateid', 'desc');
        return $this->db->get('tblemailtemplates')->result();		
	}

    public function l_get_letter(){
		$this->db->where('type', 'letters');
		$this->db->where('slug', 'lead-letter');
