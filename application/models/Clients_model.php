...
class Clients_model extends App_Model
{
	...
	public function c_get_select_letter($id){
		$this->db->where('client_id', $id);
        return $this->db->get('tblclient_letter')->result();			
	}
	
	public function c_get_email_letter(){
		$this->db->where('language', 'english');
		$this->db->where('active', 1);
		$this->db->where('type', 'letters');
		$this->db->where('slug', 'landlord-letter');
		$this->db->order_by('emailtemplateid', 'desc');
        return $this->db->get('tblemailtemplates')->result();		
	}

    public function c_get_letter(){
		$this->db->where('type', 'letters');
		$this->db->where('slug', 'landlord-letter');
