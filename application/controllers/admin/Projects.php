...
class Projects extends AdminController
{
	...
	public function property_pdf($property_id)
	{
		$html .= $this->projects_model->property_pdf_details($property_id);
        $this->pdf->createPDF($html, 'Property-'.$property_id.'', false);
	}	
	
	public function letter_pdf($letter_id)
	{
		$html .= $this->projects_model->letter_pdf_details($letter_id);
        $this->pdf->createPDF($html, 'Property-'.$letter_id.'', false);
	}	
	public function letter_update($id,$pid){
       ...
       $dataas=array(
		 ...
		 'project_id' => $pid,
		 'letterselect_id' => $this->input->post('selectitem'),
		 'select_contents' => $this->input->post('pro_letter'),
		);		
        $this->db->where('id', 1);
		$this->db->update('tblletter_selects',$dataas);		
		redirect(admin_url('projects/view/' . $pid.'?group=project_letter'));
	}
	
	//custom property image
	public function gallery_insert($id){
		$this->projects_model->insert_gallery_data($id);	
	}

	public function filteron($id){
	...
	public function floorplan_insert($id){
		$this->projects_model->insert_floorplan_data($id);	
	}
		
	
	public function gallery_delete($id){
		$this->db->where('id', $id);
		$data = $this->db->get('tblproject_gallery');
		foreach($data->result() as $row)
		 $pid=$row->project_id;
		 $fileimg=$row->gallery_img;
		{ }
		if(file_exists("uploads/property/".$fileimg))
		 {
			unlink("uploads/property/".$fileimg);
	     }
		$this->db->where('id', $id);
		$this->db->delete('tblproject_gallery');
		redirect(admin_url('projects/view/' . $pid.'?group=project_gallery'));
	}
	
	public function floorplan_delete($id){
		$this->db->where('id', $id);
		$data = $this->db->get('tblproject_foorplan');
		foreach($data->result() as $row)
		 $pid=$row->project_id;
		 $fileimg=$row->floor_img;
		{ }
		if(file_exists("uploads/property/".$fileimg))
		 {
			unlink("uploads/property/".$fileimg);
		...
		$this->db->where('id', $id);
		$this->db->delete('tblproject_foorplan');
		redirect(admin_url('projects/view/' .$pid.'?group=project_floorplan'));
	}	
    ...
    public function view($id)
    {
        if (staff_can('view', 'projects') || $this->projects_model->is_member($id)) {
            ...
            } elseif ($group == 'project_gallery') {
                  $data['all_gallery'] = $this->projects_model->get_gallerys($id);
                  //$this->load->view('admin/projects/project_gallery',$data);
            } elseif ($group == 'project_floorplan') {
                 $data['all_floorplan'] = $this->projects_model->get_floorplan($id);
            } elseif ($group == 'project_mapping') {
                $data['all_mapping'] = $this->projects_model->get_mapping($id);
            } elseif ($group == 'project_letter') {
				$data['email_letter'] = $this->projects_model->get_email_letter();
				$data['select_letter'] = $this->projects_model->get_select_letter($id);
                $data['all_letter'] = $this->projects_model->get_letter();
