...
class Emails_model extends App_Model
{
	...
	public function get_emails_info_all(){
	...
	public function insert_email_tem_data(){
        $data=array(
		
		'type' => 'letters',
		'slug' => $this->input->post('template_slug'),
		'language' => 'english',
     ...
     * @return boolean
     */
    public function mark_as($id, $enabled)
    {
		$this->db->where('language', 'english');
        $this->db->where('emailtemplateid', $id);
	...
	public function email_tem_delete($id){
		$this->db->where('language', 'english');
        $this->db->where('emailtemplateid', $id);
