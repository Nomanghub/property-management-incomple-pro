...
class Projects_model extends App_Model
{
	...
	public function letter_pdf_details($letter_id)
	{
		$this->db->where('emailtemplateid', $letter_id);
		$this->db->where('language','english');
		$data = $this->db->get('tblemailtemplates');
		$output = '<body>';
	...
	public function property_pdf_details($property_id)
	{
    ...
    //custom sql_query
	public function get_select_letter($id){
		$this->db->where('project_id', $id);
        return $this->db->get('tblletter_selects')->result();			
	}
	
	public function get_email_letter(){
		$this->db->where('language', 'english');
		$this->db->where('active', 1);
		$this->db->where('type', 'letters');
		$this->db->where('slug', 'property-letter');
    ...
    public function get_gallerys($id){
        ...
        return $this->db->get('tblproject_gallery')->result();
    }		
    ...
    public function get_floorplan($id){
        $this->db->where('project_id', $id);
		$this->db->order_by('id', 'desc');
        return $this->db->get('tblproject_foorplan')->result();
    }	
	
    public function get_mapping($id){
        $this->db->where('project_id', $id);
        return $this->db->get('tblproject_mapping')->result();
    }	
	...
	public function get_tenant_pro(){
		$this->db->order_by('id', 'desc');
        return $this->db->get('tbltenants')->result();		
	}
	
	public function insert_gallery_data($id){
		$config = array(
			'file_name'=>time(),
			'overwrite'=>TRUE,
			'max_width'=>'4028',
		...
		if($this->upload->do_upload('gallery_img')){
			...
			//redirect(admin_url('projects/view'));
		...
		$this->db->insert('tblproject_gallery',$data);	
        //$insert_id = $this->db->insert_id();		
		redirect(admin_url('projects/view/' .$id.'?group=project_gallery'));
	}
	
	
	public function insert_floorplan_data($id){
		$config = array(
			'file_name'=>time(),
			'overwrite'=>TRUE,
			'max_width'=>'4028',
		...
		$this->upload->initialize($config); 
		if($this->upload->do_upload('floorplan_img')){
			 $imageData = $this->upload->data();
			 $fileName = $imageData['file_name'];
		}else{
			//redirect(admin_url('projects/view'));
		...
		$this->db->insert('tblproject_foorplan',$data);	
        //$insert_id = $this->db->insert_id();		
		redirect(admin_url('projects/view/' .$id.'?group=project_floorplan'));
	}	
	//custom property image end
    ...
    public function add()
    {
        ...
        $dataop=array(
		 ...
		 'pro_bedrooms' => $this->input->post('pro_bedrooms'),
		 'pro_bathrooms' =>$this->input->post('pro_bathrooms'),
		 'pro_unitsize' => $this->input->post('pro_unitsize'),
		 'pro_amenities' => $this->input->post('pro_amenities'),
		 'pro_country' => $this->input->post('pro_country'),
		 'pro_city' => $this->input->post('pro_city'),
		 'pro_postcode' =>$this->input->post('pro_postcode'),
		 'pro_insurance' => $this->input->post('pro_insurance'),
		 'pro_image' => $fileName,
		 
		);
        $this->db->insert(db_prefix() . 'project_alloptions', $dataop);	
		$datamap=array(
		  'project_id' =>$insert_id,
		  'address' => $this->input->post('pro_location'),
		  'lat' => $this->input->post('latitude'),
		  'lng' => $this->input->post('longitude'),
		);
		$this->db->insert(db_prefix() . 'project_mapping', $datamap);		
		$dataav=array(
		  'project_id' =>$insert_id,
		  'name' => 'available_features',
		  'value'  =>'a:21:{s:16:"project_overview";i:1;s:13:"project_tasks";i:1;s:18:"project_timesheets";i:0;s:18:"project_milestones";i:0;s:13:"project_files";i:1;s:19:"project_discussions";i:1;s:17:"project_contracts";i:1;s:17:"project_proposals";i:1;s:17:"project_estimates";i:1;s:16:"project_invoices";i:1;s:21:"project_subscriptions";i:1;s:16:"project_expenses";i:1;s:20:"project_credit_notes";i:1;s:13:"project_notes";i:1;s:15:"project_gallery";i:1;s:17:"project_floorplan";i:1;s:15:"project_mapping";i:1;s:14:"project_letter";i:1;s:13:"project_gantt";i:0;s:15:"project_tickets";i:0;s:16:"project_activity";i:0;}'
		  
		);
		$this->db->insert(db_prefix() . 'project_settings', $dataav);
		
		redirect(admin_url('projects/view/' . $insert_id));
    }
