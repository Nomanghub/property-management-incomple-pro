...
class Emails extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['letters'] = $this->emails_model->get([
            'type'     => 'letters',
            'language' => 'english',
        ]);
    ...
    public function enable($id)
    {
        if (has_permission('email_templates', '', 'edit')) {
            ...
            $this->emails_model->mark_as($id, 1);
        }
        redirect(admin_url('emails'));
    }
    ...
    public function disable($id)
    {
        if (has_permission('email_templates', '', 'edit')) {
            $template = $this->emails_model->get_email_template_by_id($id);
            $this->emails_model->mark_as($id, 0);
        }
	...
	public function add_new_template(){
		$data['all_emaill_info'] = $this->emails_model->get_emails_info_all();
		$data['title']    = _l('Add New Email Template');
        $this->load->view('admin/emails/addnewtemplate',$data);
	}
	
	
	public function insert_template(){
		$this->emails_model->insert_email_tem_data();
		redirect(admin_url('emails'));
	}
	
	public function email_delete($id){
		$this->emails_model->email_tem_delete($id);
		redirect(admin_url('emails'));		
	}
}
