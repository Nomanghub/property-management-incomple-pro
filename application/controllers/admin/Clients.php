...
class Clients extends AdminController
{
	...
	public function  letter_update($id,$pid){
       ...
       $dataas=array(
        ...
        $this->db->where('id', 1);
		$this->db->update('tblclient_letter',$dataas);		
		redirect(admin_url('clients/client/' . $pid.'?group=letters'));
	}
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            }elseif ($group == 'letters') {
				$data['c_email_letter'] = $this->clients_model->c_get_email_letter();
				$data['c_select_letter'] = $this->clients_model->c_get_select_letter($id);
                $data['c_all_letter'] = $this->clients_model->c_get_letter();
