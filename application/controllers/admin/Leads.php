...
class Leads extends AdminController
{
	...
	public function letters($id){
		$data['l_view_data'] = $this->leads_model->l_get_viewdata($id);
		$data['l_email_letter'] = $this->leads_model->l_get_email_letter();
		$data['l_select_letter'] = $this->leads_model->l_get_select_letter($id);
		$data['l_all_letter'] = $this->leads_model->l_get_letter();		
        $data['title']    = _l('Lead Letters');
        $this->load->view('admin/leads/letters',$data);
	}
	...
	public function letter_update($id,$pid){
       ...
       $dataas=array(
        ...
        $this->db->where('id', 1);
		$this->db->update('tblleads_letter',$dataas);		
		redirect(admin_url('leads/letters/' . $pid));
	}	
